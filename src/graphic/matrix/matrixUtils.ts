...
function getMatrixElementPos(
  x: number,
  y: number,
  offset?: { logicalColCount: number; fullRow: number }
) {
  ...
  if (offset && offset.fullRow !== 0) {
    ...
    // 计算满列宽度
    const totalRowWidth =
      logicalColCount * ELEMENT_WIDTH + (logicalColCount - 1) * MATRIX_GAP;

    // 当前行元素实际宽度
    const currentRowWidth =
      fullRow * ELEMENT_WIDTH + (fullRow - 1) * MATRIX_GAP;
...
export function fillMatrixElement(
  groupId: string,
  row: number,
  col: number,
  oId?: string
...
export function drawMatrixGroup(
...
function drawGroupName(
  ...
  ctx.fillText(
    `区域名称：${group.group_name}`,
    x + w / 2,
    y + h - (transformation ? scaleSize(MATRIX_GAP) : MATRIX_GAP)
  );
}
...
export function drawGroupMatrixElement(
  ctx: CanvasRenderingContext2D,
  element: Element,
  group: Group
) {
...
export function addMatrixGroupElement(
  groupTree: Group,
  element: Element,
    ...
    if (el.pos![0] === referenceEl.pos![0]) {
      switch (type) {
        case "before": // 向前插入将基准元素以及后续元素[x,y] y + num
          if (el.pos![1] >= referenceEl.pos![1]) el.pos![1] = el.pos![1] + num;
          break;
  ...
  for (let i = 1; i <= num; i++) {
    ...
    switch (type) {
      case "before":
        pos = [referenceEl.pos![0], referenceEl.pos![1] + i - 1];
        break;
      case "after":
    ...
    for (let x = 0; x < logicalColCount; x++) {
      if (posMap.has(`${y},${x}`)) {
        ...
        if (el) {
          ...
          const { gX, gY } = getMatrixElementPos(relativeX, y, {
            logicalColCount,
            fullRow,
          });
          el.x = gX;
