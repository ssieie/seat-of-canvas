...
class OperateGraphic {
  ...
  // 更新组名称
  updateGroupName(groupId: string, name: string, setName: string) {
    const group = store.getGraphicGroupsById(groupId)
    if (!group) return
    group.group_name = name
    if (setName && group.group_set_id){
      getIdenticalSetGroups(group.group_set_id).forEach(group => {
