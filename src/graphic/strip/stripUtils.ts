...
import RuntimeStore, {
  rebuildGroupTree,
} from "../../runtimeStore/runtimeStore";
import { generateUuid, deepCopy } from "../../utils/common";
...
export function fillStripElement(
  groupId: string,
  shortNum: number,
  longNum: number
): {
  ...
  for (let i = 0; i < shortNum; i++) {
    ...
    const gX =
      ELEMENT_WIDTH + MATRIX_GAP + i * ELEMENT_WIDTH + MATRIX_GAP * i + E_GAP;
    const gY = tBasicY;
  ...
  // 右
  const rBasicX =
    elements[`${groupId}-top-${shortNum - 1}`].x + ELEMENT_WIDTH + MATRIX_GAP;
  for (let i = 0; i < longNum; i++) {
    const id = `${groupId}-right-${i}`;
    const gX = rBasicX;
    const gY =
      ELEMENT_HEIGHT + MATRIX_GAP + i * ELEMENT_HEIGHT + MATRIX_GAP * i + E_GAP;
    elements[id] = createEmptyElement(id, groupId, index, gX, gY, "right", i);
    index++;
  }
  // 下
  const bBasicY =
    elements[`${groupId}-right-${longNum - 1}`].y + ELEMENT_WIDTH + MATRIX_GAP;
...
export function drawStripGroup(
...
function drawStripGroupEmptyRegion(
  ctx: CanvasRenderingContext2D,
  x: number,
  y: number,
  w: number,
  ...
  const elRect = transformation
    ? scaleSize(ELEMENT_WIDTH + GROUP_GAP)
    : ELEMENT_WIDTH + GROUP_GAP;
  const eRect = {
    x: x + elRect,
    y: y + elRect,
    w: w - elRect * 2,
...
function drawGroupName(
  ...
  if (group.group_set_id) {
    ctx.fillText(
      `${group.group_set_name}`,
      centerOfAStripPos.x,
      centerOfAStripPos.y - scaleSize(group.baseFontSize) / 2
    );
    ctx.fillText(
      `${group.group_name}`,
      centerOfAStripPos.x,
      centerOfAStripPos.y + scaleSize(group.baseFontSize)
    );
  } else {
    ctx.fillText(
      `${group.group_name}`,
...
export function drawGroupStripElement(
  ctx: CanvasRenderingContext2D,
  element: Element,
  group: Group
) {
...
export function addStripGroupElement(
  groupTree: Group,
  element: Element,
  type: IncreaseElementPos,
  num: number
) {
  const referenceEl = deepCopy(store.getGraphicGroupElementById(element.id));
  ...
  for (let i = 1; i <= num; i++) {
    ...
    newElements.push(
      createEmptyElement(id, groupTree.group_id, 0, 0, 0, pos, idx, ``)
    );
  }
  ...
  graphicMatrix.groupElements[groupTree.group_id] = [
    ...graphicMatrix.groupElements[groupTree.group_id],
    ...newElements.map((v) => v.id),
  ];
...
export function updateStripGroupLayout(groupId: string) {
  ...
  let index = 1;
  for (const el of elements
    .filter((v) => v.strip!.pos === "top")
    .sort((a, b) => a.strip!.idx - b.strip!.idx)) {
    const base = tMax - sideLen.top;
    const gX =
      base * ((ELEMENT_WIDTH + MATRIX_GAP) / 2) +
      ELEMENT_WIDTH +
      MATRIX_GAP +
      el.strip!.idx * ELEMENT_WIDTH +
      MATRIX_GAP * el.strip!.idx +
      E_GAP;
    const gY = E_GAP;
    el.x = gX;
    el.y = gY;
  ...
  for (const el of elements
    ...
    const gX = rBasicX;
    const gY =
      base * ((ELEMENT_HEIGHT + MATRIX_GAP) / 2) +
      ELEMENT_HEIGHT +
      MATRIX_GAP +
      el.strip!.idx * ELEMENT_HEIGHT +
      MATRIX_GAP * el.strip!.idx +
      E_GAP;
    el.x = gX;
    el.y = gY;
  ...
  for (const el of elements
    .filter((v) => v.strip!.pos === "bottom")
    .sort((a, b) => b.strip!.idx - a.strip!.idx)) {
    const bBasicY =
      ELEMENT_HEIGHT +
      MATRIX_GAP +
      lMax * ELEMENT_HEIGHT +
